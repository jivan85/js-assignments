...
function getRectangleString(width, height) {
    const startLine='┌'.padEnd(width-1, '─').concat('┐\n');
    const middleLine = '│'.padEnd(width-1, ' ').concat('│\n');
    const endLine = '└'.padEnd(width-1, '─').concat('┘\n');
    const middlePart = middleLine.repeat(height-2);
    return startLine.concat(middlePart).concat(endLine);
}
